        ...
        try {
            ...
            for (IIngredientType<?> type : jeiRuntime.getIngredientManager().getRegisteredIngredientTypes()) {
                if (type.getIngredientClass().getName().equals(typeName)) {
                    // JEI 20 removed direct UID lookup. Resolve the stored UID against the
                    // registered ingredients for its type instead.
                    @SuppressWarnings({"rawtypes", "unchecked"})
                    IIngredientHelper helper = jeiRuntime.getIngredientManager()
                            .getIngredientHelper((IIngredientType) type);
                    @SuppressWarnings({"rawtypes", "unchecked"})
                    Iterable<?> ingredients = jeiRuntime.getIngredientManager()
                            .getAllIngredients((IIngredientType) type);
                    for (Object ingredient : ingredients) {
                        Object uid = helper.getUid(ingredient, UidContext.Ingredient);
                        if (uid != null && value.equals(uid.toString())) {
